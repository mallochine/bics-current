...
static int timestamp_printed = 0;
...
/*
	...
	*/
	...
	int retval;
	retval = one_command(p, astring2, cmd);
		if (retval != COM_OK) return retval;
	return COM_OK;
}
...
static int process_password(int p, char *password)
{
  ...
  if (pp->passwd && CheckPFlag(p, PFLAG_REG)) {
    ...
    if (strcmp(chessd_crypt(password,salt), pp->passwd)) {
      ...
      player_clear(p);
      pp->logon_time = pp->last_command_time = (int)time(0);
...
int process_input(int fd, char *com_string)
{
  ...
  if (com_string[0] != '$' && com_string[1] != '$') { /* wont reset idle time */
  pp->last_command_time = (int)time(0); }
...
int process_disconnection(int fd)
{
  ...
  if (BoolCheckPFlag(p, PFLAG_TOURNEY)) { SetPFlag(p, PFLAG_TOURNEY, 0); }
  pp->busy = '\0';
...
//    pcommand (p,"tell 0 I am logging out now - conversation forwarding stopped.");
  ...
  if (pp->status == PLAYER_PROMPT) {
    ...
    if (CheckPFlag(p, PFLAG_REG)) {
      pp->totalTime += (int)time(0) - pp->logon_time;
      player_save(p);
...
//
//		int now_t = tenth_secs2();
//
//		if ((now_t - gg->lastMoveTime) > gg->wRealTime)
//		{
//			if (player_globals.parray[gg->white].timeseal_pending == FLAG_NONE &&
//				gg->game_state.onMove == WHITE){
//
//				check_flag(g, WHITE);
//			}
//		}
//		if ((now_t - gg->lastMoveTime) > gg->bRealTime)
//		{
//			if (player_globals.parray[gg->black].timeseal_pending == FLAG_NONE &&
//				gg->game_state.onMove == BLACK){
//
//				check_flag(g, BLACK);
//			}
//		}
//	}
    ...
    if (nowtm->tm_min==30 || nowtm->tm_min==0) {
	...
	for (p1 = 0; p1 < player_globals.p_num; p1++) {
		...
		if (player_globals.parray[p1].status != PLAYER_PROMPT) {
			...
			if (CheckPFlag(p1, PFLAG_REG))
				player_globals.parray[p1].totalTime += (int)time(0) - player_globals.parray[p1].logon_time;
			player_save(p1);
		}
	}
	destruct_pending();
