...
SWITCH = true; //SWITCH BETWEEN ASSUND FUNCTION AND MOVEFORWARD FUNCTION
COUNTER = 0; // COUNTER OF HALFMOVES
FLAG = 1; // COUNTER INCREMENT OR DECREMENT
...
tmp+='<p><strong>BFEN A:</strong><input type="text" class="text" size="70"';
tmp+=' name="bfena"></p>';
tmp+='<p><strong>BFEN B:</strong><input type="text" class="text" size="70"';
tmp+=' name="bfenb"></p>';
...
tmp+=' window. </p>';
tmp+=' <p><input type="button" class="button" name="confirm" value="Close"';
...
tmp+='<p><strong>BFEN B:</strong><input type="text" class="text" size="70"';
tmp+=' name="bfenb"></p>';
tmp+='<p><input type="button" class="button" name="confirm" value="Load" ';
...
tmp+=' <input type="button" class="button" name="confirm" value="Close"';
tmp+=' onclick="window.close();"></p>';
tmp+='</form></body></html>';
return tmp;
}
...
else {tmp=tmp+' <input type="text" class="text" size="25" name="up'+bd+'" onFocus="this.blur();"> '};
...
else {tmp=tmp+' <input type="text" class="text" size="5" name="'+'upclock'+bd+'" onFocus="this.blur();"> '};
tmp=tmp+'<div><div  style="display:inline;">Lag </div><div style="display: inline;"id="'+this.viewername+'Lagb'+bd+'">0:00.0</div></div>';
tmp=tmp+'</td> </tr> </table>';
tmp=tmp+ '<center><table border="0"> <tr> <td rowspan="2">';
var t="";
var i;
var j;
var objn=this.viewername+'.'+bd;
var ind;
t+='<TABLE class="board" BORDER=0 CELLSPACING=0 CELLPADDING=0 COLS=8>';
for (i=0; i<8;i++)
{
t+='<tr>';
for (j=0; j<8 ; j++)
...
{k=i*2+j; s='h'+bd+k; t+='<input type="text" class="text" size="1" name="'+s+'" value=" " onFocus="this.blur();">';}
...
if (this.displaymode!="diagram") {tmp+=' <input type="text" class="text" size="18" name="dn'+bd+'" onFocus="assexecmove(document.'+this.viewername+'.lastmove'+bd+'.value,' +"'"+bd+"','"+this.viewername+"'"+');  this.blur();"> '}
else {tmp+=' <input type="text" class="text" size="25" name="dn'+bd+'" onFocus="this.blur();"> '};
};
if (this.displaymode!='diagram') {tmp+='<br/><br/><input type="text" class="text" size="10" name="lastmove'+bd+'" value=" " onChange="assexecmove(this.value,\''+bd+'\',\''+this.viewername+"'"+')" >';};
...
else {tmp=tmp+' <input type="text" class="text" size="5" name="'+'dnclock'+bd+'" onFocus="this.blur();"> '} ;
tmp=tmp+'<div ><div  style="display:inline;">Lag </div><div style="display: inline;" id="'+this.viewername+'Lagw'+bd+'">0:00.0</div></div>';
tmp=tmp+'</td> </tr> </table>';
return tmp;
}
...
function formatLag(lagString,lag) {
	
  	lagReady = lagString.split(":");
	lagReady[0]=parseInt(lagReady[0])*600;
	
	lagReady[1] = lagReady[1].split(".");
	lagReady[1][0] = lagReady[1][0]*10;
	lagReady = parseInt(lagReady[0] + lagReady[1][0] + parseInt(lagReady[1][1]) +parseInt(lag));
	lagSeconds = parseInt(lagReady%600/10).toString();
	if (lagSeconds.length == 1) {lagSeconds = "0" + lagSeconds;}
	lagReady = parseInt(lagReady/600)+":"+lagSeconds + "." + parseInt(lagReady%600)%10;
	return lagReady;
...
var t=eval('window.document.'+this.viewername); var i; var s; var k; var outc; var upLag; var downLag;

s=eval('document.'+this.viewername+'upmv'+bd); /* show whose move is now */
k=eval('document.'+this.viewername+'dnmv'+bd);

upLag = document.getElementById(this.viewername+'Lagw'+bd); 
downLag = document.getElementById(this.viewername+'Lagb'+bd);
if (FLAG==-1) {a= upLag; upLag=downLag; downLag=a;}
if (upmove) {s.src=this.gifs["mv"];k.src=this.gifs["nmv"];
upLag.innerHTML =formatLag(upLag.innerHTML,this.lagContainer[COUNTER]*FLAG);
COUNTER++;
FLAG =1;
}
else {k.src=this.gifs["mv"];s.src=this.gifs["nmv"];
downLag.innerHTML =formatLag(downLag.innerHTML,this.lagContainer[COUNTER]*FLAG); 
COUNTER++;
FLAG =1;
};
...
var v=eval(viewer);
v.allowexec=false;
while (f<num)// num - number of moves to retract
{
if (v.currentmove==BPGN_ROOT) {v.allowexec=true;return;};
mbd=v.BPGN[v.currentmove].dmove.board;// what board? (a:b)
FLAG = -1;
if (SWITCH) {
	COUNTER +=FLAG;
	SWITCH =!SWITCH;
...
out.value=out.value.replace("A.",".");
out.value=out.value.replace("B.",".");
out.value=out.value.replace("a.","... ");
out.value=out.value.replace("b.","... ");
...
var out=eval('document.'+this.viewername+'.lastmove'+bd);
out.value='';
if (whosmove(tbd.turn)=='w') {tbd.wclock=mv.backuptime1} else {tbd.bclock=mv.backuptime1}; //backs time depending on color
if (whosmove(obd.turn)=='w') {obd.wclock=mv.backuptime} else {obd.bclock=mv.backuptime}; //backs time depending on color
this.refreshclock();
this.refreshinfo();
}
...
var bc=5;
var t='<center><table border="0" cellpadding="0" cellspacing="0" width="'+parseInt((vd)-50)+'" bgcolor="'+color+'">';
t+='<tr>';

if (this.numboard!=1) {
t+='<td align="center" valign="top">';
t+='<input type="button" class="button" value=" &lt;&lt;" onclick="assundomove('+bc+','+"'a','"+this.viewername+"'"+')">';
t+='<input type="button" class="button" value=" &lt; " onclick="assundomove(1,'+"'a','"+this.viewername+"'"+')">';
t+='<input type="button" class="button" value=" &gt; " onclick="assforward(1,'+"'a','"+this.viewername+"'"+',0)">';
t+='<input type="button" class="button" value="&gt;&gt; " onclick="assforward('+bc+','+"'a','"+this.viewername+"'"+',0)">';
t+='</td>';};
t+='<td align="center" valign="bottom">';
t+='<input type="button" class="button" value=" |< " onclick="assundomove(999,'+"'c','"+this.viewername+"'"+')">';
t+='<input type="button" class="button" value=" &lt; " onclick="assundomove(1,'+"'c','"+this.viewername+"'"+')">';
t+='<input type="button" class="button" name="play" value="Play" onclick="assplay('+"'"+this.viewername+"'"+')">';
t+='<input type="button" class="button" value=" &gt; " onclick="assforward(1,'+"'c','"+this.viewername+"'"+',0)">';
t+='<input type="button" class="button" value=" >| " onclick="assforward(999,'+"'c','"+this.viewername+"'"+',0)">';
t+='</td>';
t+='<td align="center" valign="bottom">';
t+='<input style="display: none;" type="button" class="button" value=" X " onclick="assdelete('+"'"+this.viewername+"'"+')">';
var bz;
if (this.capturemode=='bug') {bz='B=>Z';};
if (this.capturemode=='zh') {bz='Z=>C';};
...
t+='<input type="button" class="button" value=" &lt;&lt;" onclick="assundomove('+bc+','+"'b','"+this.viewername+"'"+')">';
t+='<input type="button" class="button" value=" &lt; " onclick="assundomove(1,'+"'b','"+this.viewername+"'"+')">';
t+='<input type="button" class="button" value=" &gt; " onclick="assforward(1,'+"'b','"+this.viewername+"'"+',0)">';
t+='<input type="button" class="button" value="&gt;&gt; " onclick="assforward('+bc+','+"'b','"+this.viewername+"'"+',0)">';
t+='</td>';};
t=t+'</tr>';
t+='<br><tr><td><br><input type="button" class="button" value="search" id="searcha" onclick="search(this);"></td>';
t=t+'<td colspan=2 align=center>';
t+='<input type="button" class="button" value="Flip" onclick="//flipboard('+"'"+this.viewername+"'"+')">';
t+='<input type="button" class="button" value="Save" onclick="asssave('+"'"+this.viewername+"'"+')">';
t+='<input type="button" class="button" value="Load" onclick="assloadgame(\''+this.viewername+'\')"></td>';
t+='<td align="right"><br><input type="button" class="button" value="search" id="searchb"  onclick="search (this);"></td>';
t+='</tr></table>';
return t;
}
function search (board) {
b = board.id.slice("search")[6]; 
b = eval("v1."+b).generatebfen();
b=b.toString();
request = "search.php?fen=" + b;
...
t=t+'<tr>';
t=t+'<td style="display: none;" align="center" valign="top"><strong>Next</strong></td>';
t=t+'<td style="display: none;" align="center" valign="center"><strong>Comment</strong></td>';
t=t+'</tr>';
t=t+'<tr>';
t=t+'<td valign="top">';
t+='<select name="nextmove" size="1" onChange="assforward(1,'+"'c','"+this.viewername+"'"+',document.'+this.viewername+'.nextmove.selectedIndex);" >';
for(i=0;i<MAX_NEXT;i++) opt+='<option> ';
t=t+'<option>'+ph+'</option>'+opt;
t=t+'</select></td>';
t=t+'<td valign="top"><div valign=top>Comments:</div><textarea name="comment"'+ ((IE)?' ':' WRAP="SOFT" ')+ 'value="" rows="3" cols="'+this.cwidth+'"></textarea></td>';
t=t+'</tr>';
t=t+'</table></center>';
return t;
}
...
if ((l>0)&&(isnum(t[l-1]))) {this.bclock=parseInt(t[l-1]);};
if ((l>1)&&(isnum(t[l-2]))) {this.wclock=parseInt(t[l-2]);};
...
function board(name,dropbar,gifs,viewername,whitepl,blackpl,welo,belo,displaym,reloadm, bfen, timecontrol, flip,size, file_bg, redrawmode, lagContainer)
{
this.lagContainer = lagContainer;
this.filebg=file_bg;
...
function Lag_array (lagContainer) {
  lagContainer = "0 0 "+lagContainer;
  
  var lagContainer = lagContainer.split(" ");
  for (x =0; x< lagContainer.length; x++) {
    if (lagContainer[x] != 0) {
	  
	  lagContainer[x] = parseInt(parseInt(lagContainer[x])/100);
...
function game(the_viewer_name, bpgntext, bfen_pos, capture_mode, number_of_boards, display_mode, reload_mode, gif_file_path, sq_size, bg_color,flip, lagContainer) /* constructor of the game object */
{
...
if (bg_color.length<1) {bg_color="white"};

lagContainer = Lag_array(lagContainer);
...
this.a=new board("a", this.dropbar,this.gifs,this.viewername,this.whitea,this.blacka,this.whiteaelo,this.blackaelo,this.displaymode,this.reloadmode, s[0], this.gametime, this.flip,size,filebg,true,lagContainer);

var redrawmode= (this.numboard==1)?false:true;
this.b=new board("b", this.dropbar,this.gifs,this.viewername,this.whiteb,this.blackb,this.whitebelo,this.blackbelo,this.displaymode,this.reloadmode, s[1], this.gametime, (this.flip+1) %2,size,filebg, redrawmode,lagContainer );
