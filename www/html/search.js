...
function RequestObject_SendRequest(hook,action) {
	var reqid = 'req'+Math.random();//didomir
	//var reqid = 'req'+"0.1131288766618942";//didomir
	var u = this.script+'?obid='+this.ownerid+'&reqid='+reqid+'&hook='+hook+'&action='+action;
	//document.write(u); //debug didomir
	if (this.queue.length == 0) {
		try {
			this.queue.push(u);
...
function Objectmap_Add(baseid,ob) {
	var theid = baseid + '' + Objectmap.objectcount + Math.random(); //didomir
	//var theid = baseid + '' + Objectmap.objectcount + "0.9737026394117703";
...
function Position_CopyFrom(p) {
	...
	o.halfmovecounter = p.halfmovecounter;
} 
...
function Position_ClearHistory(cc,ce) {
	...
	o.lastmove = 0; o.lastmovetext = null; } 
...
function Position_SetFEN(fen) {
 ...
 if (o.ep>0&&o.wtm) o.ep+=24; o.halfmovecounter = tok.length>4?eval(tok[4]):0; o.pliesplayed = (tok.length>5?2*eval(tok[5]):2) - (o.wtm ? 2 : 1); o.lastmove = 0; o.lastmovetext = null;} catch (e) {} o.PositionChanged();} 
...
function Position_MakeMove(f,t,p) {
 var o=this,c=o.castling; if (o.makemovetext) {o.lastmovetext=o.GetMoveString(f|(t<<6)|(p<<12));} if (t==o.ep&&(o.b[f]==6||o.b[f]==12)) {o.b[o.ep+(o.wtm?-8:8)]=0;}o.b[t]=o.b[f]; o.b[f]=0; 
 ...
 o.pliesplayed++; o.lastmove=(f|(t<<6)|(p<<12)); o.PositionChanged();} 

function Position_PositionChanged() {this.moves=null;for (var i = 0; i < this.listeners.length; i++)	this.listeners[i].PositionChanged(this);}

function Position_AddListener(listener) {	this.listeners.push(listener);} 

function Position_NumberOfPieces() { var r = 0; for (var i = 0; i < 64; i++) if (this.b[i] > 0) r++; return r; } 

function Position_GetMoves() {if (this.moves!=null)return this.moves; this.moves = MG.FindMoves(this);return this.moves;} 
...
function EndgameTable_SetHeaderTable(lang) {
	...
	else texts = new Array("Move","Games");
...
function EndgameTable_SetObserver(board) {
		this.observer = board; //added by didomir
}
...
function EndgameTable_SetData(moves) {
	...
	for (var k = 0; k < moves.length; k++) {
		var lengths = new Array(100,100);
		var thestyle = 'style="font-family:Verdana,Geneva,Arial,Helvetica,sans-serif; font-size:11px;font-weight:100;cursor:pointer;"';
		var mouseBehavior = ' onmouseout="javascript:EndgameTable.Endingout(event,this);" onmouseover="javascript:EndgameTable.Endingover(event,this);" onmousedown="javascript:EndgameTable.Endingdown(event,this)"'; //added by didomir
		txt += '<tr id="'+this.tableid+'_'+k+'"'+mouseBehavior+'>';		
		...
		var rem = toks.length > 2 ? "=<img align= auto valign = top src='diagram/"+diagramgraphics[toks[2]]+"'></img>" : '';
		var pawns ='';
		if (f==65)
		{
			if (this.position.piececodes[toks[2]]=='')
			{
				var pawns ='p';
			}
			//moves[k][0] = this.position.piececodes[toks[2]]+pawns+"@"+SQ.names[toks[1]];
			moves[k][0] ="<img align= auto valign = top	src='diagram/"+diagramgraphics[eval(toks[2])  +6-6*this.position.wtm]+"'></img>"+"@"+SQ.names[toks[1]];
			
		}
		else {
		moves[k][0] = "<img align= auto valign = top src='diagram/"+diagramgraphics[p]+"'></img>" +SQ.names[toks[0]]+mi+SQ.names[toks[1]]+rem;
		}
	...
	if (this.posval.length == 2 && this.position.wtm) 
		...
		document.getElementById(this.tableid+'_value').innerHTML = this.posval[0]; //edited by Didomir
} 
...
function EndgameTable_RequestData(pos) {
	...
	if (n >= 2 && valid) { //Maybe we need to set bigger min number of pieces
	
		var fen = pos.GetFEN();
		var t = 'bics&fen='+fen; //edited by didomir
		
		/*var tok = t.split(' '); //removed by didomir
		tok[1] = 'w';
		tok[tok.length-2] = '0';
		tok[tok.length-1] = '1';
		t = tok[0];
...
function EndgameTable_ParseRequestResult(req,text) {
	...
	if (text) {
		...
		var wtm = _t;
		for (var i = 0; i < tok.length; i++) {
			//if (tok[i] == "Black") { wtm = _f; continue; } //edited by Didomir
			...
			if (m.length==3)
			{
				m[2]=6 + eval(m[2]) - eval(this.position.wtm) *6;
			}
			this.movefrom[num] = eval(m[0]);	
			this.moveto[num] = eval(m[1]);	this.movepromo[num] = m.length == 3 ? eval(m[2]) : 0;
...
function EndgameTable_Endingdown(ev, ob) {
	...
	if (tab.position != null) {
		document.getElementById(tab.tableid+'_body').innerHTML = "Loading...";
...
function GuiCheckbox_GetState() { return this.state; } 
...
function GuiRadio_GetState() { return this.state; } 
...
function SetupTable(posx, posy, lang, position) {
	...
	this.darkcolor = "#E7CEA5";
	this.lightcolor = "#E7CEA5";
...
function SetupTable_GetDivElement() {
	...
	result += "<div id=\""+this.tableid+"_move\" style=\"position:absolute;left:"+(6*this.sqsize)+";top:0;background-color:"+this.darkcolor+"; cursor: pointer; \" onmousedown=\"SetupTable.Mousedown(event,this);\" >Move</div>\n";	
	...
	result += "<div id=\""+this.tableid+"_empty\" style=\"position:absolute;left:"+(7*this.sqsize+this.sqsize/4)+";top:0; cursor: pointer; \" onmousedown=\"SetupTable.Mousedown(event,this);\" >Start</div>\n";	
    result += "<div id=\""+this.tableid+"_start\" style=\"position:absolute;left:"+(7*this.sqsize+this.sqsize/4)+";top:"+to+"; cursor: pointer; \" onmousedown=\"SetupTable.Mousedown(event,this);\" >Initial fen</div>\n";	


   /* move pos arrows

	result += '<div class="button" id="'+this.tableid+'_u" style="position:absolute; left:'+(this.sqsize*8+13+20)+'; top:'+(0)+';width:13;height:13;" onmouseover="over(this);" onmouseout="leave(this);" onmousedown="SetupTable.Mousedown(event,this);" onclick="">&uarr;</div>';
...
function SetupTable_MarkSquare(sq) {
		...
		if (sq >= 0) {
			var f = sq & 7;
			var r = sq >> 3;
			col = ((r % 2) == (f % 2)) ? "#996600" : "#996600";
		}
		else { sq = 'move'; col="#A57B5A"; }
...
//	}
	...
	if (elem == 'empty') {
		p.SetFEN("rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w");
	}
	else if (elem == 'd') {
...
function SetupTable_SetVisible(b) {
		if (document.getElementById(this.tableid).style.visibility == 'visible')
			
		{
			document.getElementById(this.tableid).style.visibility = 'hidden';
			b.innerHTML = "Show Setup";
			st.SetPiece(-1);
			
		}
		else {
		    document.getElementById(this.tableid).style.visibility = 'visible';
			b.innerHTML = "Hide Setup";
		}
		//document.getElementById(this.tableid).style.visibility = b ? 'visible' : 'hidden';
...
//added by didomir
	function GuiInput (label,value, id) {
	    this.label = label;
	    this.id = id;
		var d = document.createElement("input");
		d.style.position = 'relative';
		d.innerHTML = "Player A rating";
		/*d.style.left = 4000;
		d.style.top = 100;
		d.style.height = 30;
		d.style.width = 250;
		...
		*/
		d.value = value;
		document.getElementById("searchForm").appendChild(d);
	}
	...
	function buildIt() {
		...
		ratinginput1 = new GuiInput("rating","1900","black");
		...
		*/
		...
		b = new Board(sqsize, "#A57B5A" , "#E7CEA5", 250, 10);
		b.SetVisible(true);	
		b.allowfreemoving = false;
		p.AddListener(b);
		b.SetRefPosition(p);
		et = new EndgameTable(10,60,279,"en",p);
		p.AddListener(et);
		et.SetVisible(true);
		et.SetObserver(b);
	...
	function over(t) {
		t.style.background = '#996600';
		t.style.color = '#E7CEA5';
	}
	function leave(t) {
		t.style.color = '#000000';
		t.style.background = '#E7CEA5';
	}
...
var diagramgraphics = new Array ("", "wk.gif", "wq.gif","wr.gif","wb.gif","wn.gif","wp.gif", "bk.gif", "bq.gif", "br.gif", "bb.gif", "bn.gif", "bp.gif");
...
var startboardgraphics = "images/a"+sqsize+"startpos.jpg";
...
Position.prototype.CopyFrom = Position_CopyFrom;
Position.prototype.SetStart = Position_SetStart;
Position.prototype.SetSetupTable = Position_SetSetupTable;
Position.prototype.Clear = Position_Clear;
Position.prototype.ClearHistory = Position_ClearHistory;
Position.prototype.SetFEN = Position_SetFEN;
var fenpiecechars = "1KQRBNPkqrbnp";
Position.prototype.GetFEN = Position_GetFEN;
Position.prototype.MakeMove = Position_MakeMove;
Position.prototype.PositionChanged = Position_PositionChanged;
Position.prototype.AddListener = Position_AddListener;
Position.prototype.NumberOfPieces = Position_NumberOfPieces;
Position.prototype.GetMoves = Position_GetMoves;
...
GuiCheckbox.prototype.GetState = GuiCheckbox_GetState;
...
GuiRadio.prototype.GetState = GuiRadio_GetState;
